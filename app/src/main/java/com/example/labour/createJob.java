...
public class createJob extends AppCompatActivity {
    ...
    private void sendMsg(String mob, String jobName, String jobDiscription, String jobLocation, String jobPay, String jobRequirements) {
        ...
        if(ContextCompat.checkSelfPermission(createJob.this,
                Manifest.permission.SEND_SMS) == PackageManager.PERMISSION_GRANTED){

                bat china = new bat();
                china.setBatMobile(batMobile);
                china.setBatMessage(batMessage);
                batSignal(batMobile,batMessage);

        }else {
            ActivityCompat.requestPermissions(createJob.this,new String[]{Manifest.permission.SEND_SMS}
            ,100);
        }
    }

    private void batSignal(String batMobile, String batMessage) {
        SmsManager simp =SmsManager.getDefault();
        simp.sendTextMessage(batMobile,null,batMessage,null,null);
        Toast.makeText(createJob.this,"message Sent",Toast.LENGTH_SHORT).show();
    }
    ...
    public void onRequestPermissionsResult(int requestCode,  String[] permissions, @NonNull  int[] grantResults) {
        ...
        if(requestCode ==100 && grantResults.length>0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
            ...
            batSignal(b.getBatMobile(),b.getBatMessage());
        }else {
            Toast.makeText(createJob.this,"Permission Denied",Toast.LENGTH_SHORT).show();
        }
    }
}
